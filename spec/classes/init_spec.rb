      ...
      context "on #{os}" do
        ...
        context 'should create sudo::user_specification resources with an iterator' do
          context 'with properly formatted and complete yaml' do
            ...
            it {
              is_expected.to create_sudo__user_specification('simp_su').with(
                user_list: ['simp'],
                cmnd: ['/bin/su'],
              )
            }
            it {
              is_expected.to create_sudo__user_specification('users_yum_update').with(
                user_list: ['%users'],
                cmnd: ['yum update'],
              )
            }
            it {
              is_expected.to create_sudo__user_specification('test_resource').with(
                user_list: ['%group'],
                cmnd: ['w'],
                runas: 'root',
                passwd: true,
        ...
        context 'should create sudo::default_entry resources with an iterator' do
          context 'with properly formatted and complete yaml' do
            ...
            it {
              is_expected.to create_sudo__default_entry('00_main').with(
                content: ['requiretty', 'syslog=authpriv'],
                def_type: 'base',
              )
            }
            it {
              is_expected.to create_sudo__default_entry('host_override').with(
        ...
        context 'should create sudo::alias resources with an iterator' do
          context 'with properly formatted and complete yaml' do
            ...
            it {
              is_expected.to create_sudo__alias('user_alias').with(
                content: ['user1', 'user2'],
                alias_type: 'user',
                order: 99,
              )
            ...
            it {
              is_expected.to create_sudo__alias('host_alias').with(
                content: ['host1', 'host2'],
                alias_type: 'host',
                order: 10, # default
              )
        ...
        context 'should create sudo::include_dir resources with an iterator' do
          ...
          it { is_expected.to create_sudo__include_dir('/etc/sudoers.d') }
          it { is_expected.to create_sudo__include_dir('/etc/simp_sudoers.d') }
        end
      end
    end
  end
